...
public class Rectangle implements Geometry2D {
  ...
  {
	  double dx = seg.A.getX() - seg.B.getX();
	  double dy = seg.A.getY() - seg.B.getY();
	  
	  if (dx == 0 && dy == 0) return null; // There is no intersection when the length is 0.
	  
	  ArrayList<LineSegment> rectLines = new ArrayList<LineSegment>();
	  rectLines.add(new LineSegment(ptMin.x, ptMin.y, ptMax.x, ptMin.y));
	  rectLines.add(new LineSegment(ptMin.x, ptMin.y, ptMin.x, ptMax.y));
	  rectLines.add(new LineSegment(ptMax.x, ptMin.y, ptMax.x, ptMax.y));
	  
	  Point2D intersectPoint = null;
	  for(int i=0; i<rectLines.size(); i++)
	  {
		  Point2D pti = seg.intersect(rectLines.get(i));
		  
		  if(pti != null && pti.x > 0 && pti.y > 0)
		  {
			  intersectPoint = pti;
		  }
	  }
